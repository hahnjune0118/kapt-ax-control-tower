...
### Power Automate 업무 자동화 데모

[▶ Power BI → SharePoint → Power Automate 업무 자동화 데모 보기](docs/assets/powerbi/KAPT_AX_Control_Tower_Portfolio_Demo_Under_5MB.mp4)

Power BI에서 선택한 관리비 이상징후를 Power Automate로 전달해 SharePoint 검토 요청을 생성하고, 담당자 배정·검토의견·처리상태 관리와 상태 변경 이메일 알림까지 연결했습니다.

`이상징후 선택 → 검토 요청 생성 → 담당자 검토 → 상태 변경 → 이메일 알림`

> Power Automate 실행에는 Microsoft 365 사용자 인증이 필요하므로 공개 Power BI 링크에서는 직접 실행되지 않습니다. 위 영상은 로그인된 Power BI Service 환경에서 수행한 end-to-end 시연입니다.
...
- Power Automate·SharePoint 기반 검토 요청 생성, 담당자 배정, 검토의견·처리상태 관리 및 이메일 알림 구현
...
- Power BI에서 선택한 이상징후를 SharePoint 검토 요청으로 생성하고, 담당자·검토의견·처리상태 관리와 상태 변경 이메일 알림까지 구현했습니다.
- 다음 확장은 예약 데이터 갱신, `Assignee` 기반 동적 이메일 수신자 연결, 증빙 첨부·승인 단계 및 사용자 피드백 기반 모델 모니터링입니다.
